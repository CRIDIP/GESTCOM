...
if(is_ajax()){
    if(isset($_GET['action']) && $_GET['action'] == 'commande_saisie')
    {
        session_start();
        require "../../application/classe.php";

        $count_c = $DB->count("SELECT COUNT(idcommande) FROM commande WHERE etat_commande = 0");

        echo json_encode($count_c);
    }
    if(isset($_GET['action']) && $_GET['action'] == 'commande_attente')
    {
        session_start();
        require "../../application/classe.php";

        $count_c = $DB->count("SELECT COUNT(idcommande) FROM commande WHERE etat_commande = 1");

        echo json_encode($count_c);
    }
    if(isset($_GET['action']) && $_GET['action'] == 'commande_valide')
    {
        session_start();
        require "../../application/classe.php";

        $count_c = $DB->count("SELECT COUNT(idcommande) FROM commande WHERE etat_commande = 2 OR etat_commande = 3");

        echo json_encode($count_c);
    }

    if(isset($_GET['action']) && $_GET['action'] == 'supp_commande')
    {
        session_start();
        require "../../application/classe.php";
        $num_commande = $_GET['num_commande'];

        $del_ligne = $DB->execute("DELETE FROM commande_article WHERE num_commande = :num_commande", array("num_commande" => $num_commande));
        $del_devis = $DB->execute("DELETE FROM commande WHERE num_commande = :num_commande", array("num_commande" => $num_commande));
    ...
    if(isset($_GET['action']) && $_GET['action'] == 'edit_status')
    {
        ...
        $devis_u = $DB->execute("UPDATE commande SET etat_commande = :etat_commande WHERE num_commande = :num_commande", array(
            "num_commande"     => $num_commande,
            "etat_commande"    => $etat
        ));

        $sql_client = $DB->query("SELECT * FROM commande, client WHERE commande.idclient = client.idclient AND num_commande = :num_commande", array("num_commande" => $num_commande));
        $client = $sql_client[0];
        
        if($etat == 1):
            $to = $client->mail_client;
            $sujet = "Votre Commande N°".$num_commande." - SAS CRIDIP";
                            ...
                            <tr>
                                <td style="color: #181c27; font-size: 30px; font-family: 'Questrial', sans-serif; mso-line-height-rule: exactly; line-height: 30px;" class="title_color main-header" align="center">
                                    ...
                                    <div class="editable_text" style="line-height: 30px;">
                                        <span class="text_container">VOTRE COMMANDE EST PRETE</span>
                                    </div>
                                </td>
                            </tr>
                                        ...
                                        <tbody><tr>
                                            <td style="color: #8d94a3; font-size: 16px; font-family: 'Questrial', sans-serif; mso-line-height-rule: exactly; line-height: 24px;" class="resize-text text_color" align="center">
                                                <div class="editable_text" style="line-height: 24px">
                                                    ...
                                                    <!-- ======= section text ======= -->
                                                    <span class="text_container">Votre commande N° <strong><?= $num_commande; ?></strong> d'un montant de <strong><?= $fonction->number_decimal($tva->calc_ttc($client->total_commande)); ?></strong> est disponible sur votre espace personnel.</span>
                                                </div>
                                            </td>
                                        </tr>
                                        </tbody></table>
        ...
        if($etat == 3):
            ...
            $fct_i = $DB->execute("INSERT INTO facture(num_commande, num_facture, idclient, date_facture, total_facture, etat_facture) VALUES (:num_commande, :num_facture, :idclient, :date_facture, :total_facture, :etat_facture)", array(
                "num_commande"  => $num_commande,
                "num_facture"   => $num_facture,
                "idclient"      => $client->idclient,
                "date_facture"  => $date_format->date_jour_strt(),
                "total_facture" => $client->total_commande,
                "etat_facture"  => 1
            ));

            $fct_q = $DB->query("SELECT * FROM facture WHERE num_facture = :num_facture", array("num_facture" => $num_facture));
            $facture = $fct_q[0];

            $sql_cmd_article = $DB->query("SELECT * FROM commande_article WHERE num_commande = :num_commande", array("num_commande" => $num_commande));
            foreach ($sql_cmd_article as $article):
                $article_i = $DB->execute("INSERT INTO facture_article(num_facture, idarticle, qte, total_ligne) VALUES (:num_facture, :idarticle, :qte, :total_ligne)", array(
                    "num_facture"   => $num_facture,
    ...
    {
        require "../../application/classe.php";
        $idcommandearticle = $_GET['idcommandearticle'];
        $num_commande = $_GET['num_commande'];

        $sql_article = $DB->query("SELECT * FROM commande_article WHERE idcommandearticle = :idcommandearticle", array("idcommandearticle" => $idcommandearticle));
        $sql_commande = $DB->query("SELECt * FROM commande WHERE num_commande = :num_commande", array("num_commande" => $num_commande));
        $total_commande = $sql_commande[0]->total_commande;

        $article_d = $DB->execute("DELETE FROM commande_article WHERE idcommandearticle = :idcommandearticle", array("idcommandearticle" => $idcommandearticle));
        $commande_up = $DB->execute("UPDATE commande SET total_commande = :total_commande WHERE num_commande = :num_commande", array(
            "num_commande"     => $num_commande,
            "total_commande"   => $total_commande - $sql_article[0]->total_ligne
        ));
